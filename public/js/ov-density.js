...
.ovd {
    ...
    border: 1px solid color-mix(in srgb, var(--sc, var(--accent)) 20%, transparent);
}
.ovd-btn {
...
.ovd-btn:hover { color: var(--text-primary); background: color-mix(in srgb, var(--sc, var(--accent)) 16%, transparent); }
.ovd-btn:active { transform: scale(.93); }
.ovd-btn[aria-pressed="true"] {
    color: color-mix(in srgb, var(--sc, var(--accent)) 92%, #fff);
    background: color-mix(in srgb, var(--sc, var(--accent)) 24%, transparent);
    box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--sc, var(--accent)) 42%, transparent);
}
.ovd-btn:focus-visible { outline: 2px solid var(--sc, var(--accent)); outline-offset: 2px; }
...
`;
    ...
    function maxColumns(el) {
        ...
        if (!w) return 1;
        return Math.max(1, Math.min(HARD_MAX, Math.floor(w / MIN_CARD)));
    }

    function attach(grid, { key, header } = {}) {
        if (!grid || !key) return;
        ...
        const render = () => {
            ...
            const active = wanted ? Math.min(wanted, max) : 0;
            ...
            const effective = active || autoColumns(grid, max);
