...
(function (root) {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .openvibe-toast-title .service-badge { font-size: 9px; padding: 1px 6px; border-radius: 3px; background: rgba(139,92,246,0.15); color: var(--accent-light, #a78bfa); font-weight: 500; }
            ...
            .openvibe-notif-item.unread { background: rgba(139,92,246,0.04); border-left-color: var(--accent, #8b5cf6); }
    ...
    function showToast(notification) {
        ...
        if (rich.user) {
            ...
            const nameClass = u.name_effect ? `openvibe-name-fx-${esc(u.name_effect)}` : '';
        ...
        toast.innerHTML = `
            <div class="openvibe-toast-icon">${esc(notification.icon) || '🔔'}</div>
            <div class="openvibe-toast-body">
                ${userHtml}
                <div class="openvibe-toast-title">${esc(notification.title)}${serviceLabel}</div>
                <div class="openvibe-toast-msg">${esc(notification.message)}</div>
                ${actionsHtml}
            </div>
    ...
    async function loadNotifications(tab = 'all') {
        ...
        try {
            ...
            list.innerHTML = `<div class="openvibe-notif-panel-empty"><span class="icon">⚠️</span>${hint}</div>`;
        }
    }
