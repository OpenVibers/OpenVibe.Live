...
if [ "$LAYOUT" = release ]; then
    ...
    if has "$CATS" nginx; then
        say "note: deploy/nginx changed. The live nginx config is generated by OpenVibe.Network's"
        say "      nginx generator and is NOT installed by this script — apply the change there and run nginx -t."
    fi
...
has "$CATS" nginx && say "note: deploy/nginx changed — apply it through OpenVibe.Network's nginx generator; not installed here."
