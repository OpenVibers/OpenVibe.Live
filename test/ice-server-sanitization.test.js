...
try {
    ...
    assert.strictEqual(iceServers.length, 1, 'Invalid TURN URL should be skipped and only the STUN server kept');
    ...
    assert.strictEqual(iceServers2.length, 3, 'Valid TURN URL should produce 2 TURN entries plus the STUN entry');
    assert.strictEqual(iceServers2[1].urls, 'turn:turn.example.com:3478');
    assert.strictEqual(iceServers2[1].username, 'user');
    assert.strictEqual(iceServers2[1].credential, 'pass');
    assert.strictEqual(iceServers2[2].urls, 'turn:turn.example.com:3478?transport=tcp');
