...
(async () => {
    ...
    const S = [...names.aliasEntries(9, ['Mater151']), ...names.aliasEntries(10, ['brunomelato'])];
    const hit2 = (t) => names.findMentions(t, S).map(m => `${m.userId}:${m.how}`);
    assert.deepStrictEqual(hit2('it doesnt matter anymore'), [], 'short names never sound-match plain words');
    assert.deepStrictEqual(hit2('the water is cold'), []); assert.deepStrictEqual(hit2('see you later'), []);
    assert.deepStrictEqual(hit2('mater is trash'), ['9:exact'], 'short names still match exactly');
    assert.deepStrictEqual(hit2('bruno melato is a bot'), ['10:phonetic'], 'a glued handle split by the transcriber');
    assert.deepStrictEqual(hit2('a bruno at the door'), [], 'half a handle is not the handle');
