...
assert.strictEqual(p.xp, ch.XP_MOMENT + ch.XP_FIRST_ON_SUBJECT + 5 + 5, 'moment + first-on-subject + day-1 streak + the Said Something achievement');
...
assert.strictEqual(p.xp, 17 + ch.XP_MOMENT, 'second line: just the moment (streak already paid today)');
...
assert.strictEqual(ch.profile(`user:${uid}`).xp, 20 + ch.XP_MOMENT_HOT + 10, 'hot subjects pay double (+ the Hot Take achievement)');
...
(async () => {
    ...
    assert.strictEqual(ch.profile(`user:${uid}`).xp, before + ch.XP_QUOTED_IN_LORE + 15, 'quoted in the lore → XP (+ the In The Lore achievement)');
