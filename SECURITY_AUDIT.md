...
| Medium | Voice: 1:1 call channels were listed publicly and anyone (anonymous included) could walk into them; `call-user` had no rate limit or block check; anyone could answer any invite; a kicked user could rejoin at once; one host could fill the 8-seat Public channel; TURN was emitted without credentials on a box with no secret; static TURN credentials went to every visitor. | Private calls hidden from strangers and invite-only; call-user 6/min + block check; respond requires the invite; 60 s kick cooldown; 3 sockets per address, duplicate accounts replaced; TURN only with credentials, optional short-lived HMAC credentials (`TURN_AUTH_SECRET`, coturn `use-auth-secret`). | manual + two-client voice test |
...
| Low | Voice TURN credentials are still the static pair until `TURN_AUTH_SECRET` is set and coturn switched to `use-auth-secret` (production change). | Configure the secret on the host. |
